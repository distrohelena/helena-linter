...
export function tryGetComplementComparison(
    ...
    if (ts.isPrefixUnaryExpression(firstUnwrappedExpression)) {
        if (
            firstUnwrappedExpression.operator !== ts.SyntaxKind.ExclamationToken
        ) {
            return undefined;
        }

        const targetExpression = unwrapExpression(
            firstUnwrappedExpression.operand,
        );
        if (
            targetExpression.getText() !== secondUnwrappedExpression.getText()
        ) {
            return undefined;
        }
        ...
        if (
            secondUnwrappedExpression.operator !==
